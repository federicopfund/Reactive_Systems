...
import domains.publications.repositories.{PublicationRepository, PublicationFeedbackRepository, PublicationStageHistoryRepository}
...
class AdminController @Inject()(
  ...
  def loginPage(): Action[AnyContent] = Action { implicit request =>
    Redirect(controllers.auth.routes.AuthController.loginPage())
  }
  ...
  def logout(): Action[AnyContent] = Action { implicit request =>
    Redirect(controllers.web.routes.HomeController.index())
      .withNewSession
      ...
      case Some(pub) =>
        for {
          ...
          ok = targetStage.isDefined &&
               EditorialStagePolicy.canTransitionFrom(currentCode, request.role) &&
               EditorialStagePolicy.isAllowedTarget(currentCode, toCode)
          result <- if (!ok) {
  ...
  def approveAdmin(userId: Long): Action[AnyContent] = superAdminAction.async { implicit request: AuthRequest[AnyContent] =>
    ...
    RolePolicy.assignableRoles.find(_.key == rawRole) match {
      case None =>
        Future.successful(
