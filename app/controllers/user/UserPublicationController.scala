...
import domains.publications.repositories.{PublicationRepository, PublicationFeedbackRepository, PublicationStageHistoryRepository, ReactionRepository, CommentRepository}
import domains.identity.repositories.UserRepository
import domains.messaging.repositories.{UserNotificationRepository, PrivateMessageRepository}
import domains.gamification.repositories.{BookmarkRepository, BadgeRepository}
...
import domains.publications.models.{Publication, PublicationStatus, PublicationComment}
import domains.messaging.models.{NotificationType, PrivateMessage}
import domains.editorial.models.EditorialStageCode
import domains.messaging.services.ReactiveMessageAdapter
...
class UserPublicationController @Inject()(
  ...
  def toggleReaction(publicationId: Long) = userAction.async { implicit request: AuthRequest[AnyContent] =>
    ...
    for {
      ...
      val referer = request.headers.get("Referer").getOrElse(controllers.web.routes.HomeController.publicaciones().url)
  ...
  def addComment(publicationId: Long) = userAction.async { implicit request: AuthRequest[AnyContent] =>
    ...
    if (content.trim.length < 3) {
      Future.successful(
        Redirect(request.headers.get("Referer").getOrElse(controllers.web.routes.HomeController.publicaciones().url))
  ...
  def sendMessage(publicationId: Long) = userAction.async { implicit request: AuthRequest[AnyContent] =>
    ...
    if (subject.isEmpty || content.isEmpty) {
      Future.successful(
        Redirect(controllers.web.routes.HomeController.publicacion(""))
