...
import domains.identity.services.{EmailVerificationService}
    ...
    request.session.get("userId") match {
      case Some(_) =>
        request.session.get("userRole") match {
          case Some("admin") => Redirect(controllers.admin.routes.AdminController.dashboard(0, None))
      ...
      case Some(user) if BCrypt.checkpw(password, user.passwordHash) =>
        if (!user.adminApproved && user.role != "super_admin") {
          ...
          userRepository.updateLastLogin(user.id.get).map { _ =>
            ...
            Redirect(controllers.admin.routes.AdminController.dashboard(0, None))
              .withSession(
