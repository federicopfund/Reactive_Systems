package domains.identity.repositories

import domains.identity.models.User
...
class UserRepository @Inject()(
   ...
   * Se basa en la lista declarada en `RolePolicy.backofficeRoleKeys`
   * para mantener una única fuente de verdad.
   */
  def findApprovedAdmins(): Future[Seq[User]] = {
    val staffRoles = RolePolicy.backofficeRoleKeys
   ...
   * Roles permitidos: cualquiera declarado en `RolePolicy.assignableRoles`.
   */
  def approveAdmin(userId: Long, approvedBy: Long, role: String = "editor_jefe"): Future[Int] = {
    val safeRole = RolePolicy.assignableRoles.find(_.key == role)
      .map(_.key)
  ...
  def changeAdminRole(userId: Long, newRole: String): Future[Int] = {
    val safeRole = RolePolicy.assignableRoles.find(_.key == newRole)
      .map(_.key)
  ...
  def revokeAdmin(userId: Long): Future[Int] = {
    val revokableRoles = RolePolicy.backofficeRoleKeys - "super_admin"
    db.run(
